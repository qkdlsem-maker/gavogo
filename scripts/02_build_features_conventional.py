#!/usr/bin/env python
"""[2단계, conventional/구 프로토콜] canonical + 이벤트 → 피처 데이터셋 (horizon별).
cross-vehicle 샘플러(build_samples, 누수 있음) 사용. 논문 paper protocol은
02_build_features.py(구 _v2.py, within-vehicle 샘플러)를 쓴다.
실행: python scripts/02_build_features_conventional.py --dataset highD|NGSIM|MiTra|ETRI|uniD|exiD|EMT"""
import argparse, sys
...
def main():
    ...
    for hsec in config.HORIZONS_SEC:
        ...
        ds[keep].to_csv(config.PROCESSED_DIR / f"{a.dataset}_conv_{hsec}s.csv", index=False)
        print(f"  [{hsec}s] {len(ds)} 샘플 "
              f"(LC={int(ds.label.sum())}, LK={int((ds.label==0).sum())}) "
              f"→ {a.dataset}_conv_{hsec}s.csv")
