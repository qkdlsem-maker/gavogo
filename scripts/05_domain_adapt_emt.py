#!/usr/bin/env python
"""[5단계-EMT] cross-domain 적응: EMT OOD 개선.
위치: scripts/05_domain_adapt_emt.py
비교: zeroshot / 표준화 / fine-tune / 표준화+fine-tune
학습: highD+NGSIM+MiTra, 타깃: EMT
실행: python scripts/05_domain_adapt_emt.py"""
import sys
...
def main():
    ...
    for h in config.HORIZONS_SEC:
        # 학습: highD+NGSIM+MiTra (TRAIN_DATASETS 원복 기준)
        src = pd.concat(
            [load(ds, h) for ds in ["highD", "NGSIM", "MiTra"]],
            ignore_index=True
        )
        emt = load("EMT", h)
        cols = [c for c in COLS if c in src.columns and c in emt.columns]

        Xs, ys = clean(src, cols), src["label"].astype(int)
        Xe, ye = clean(emt, cols), emt["label"].astype(int)
    ...
    out.to_csv(config.TABLES_DIR / "domain_adapt_emt.csv", index=False)
    print("\n저장: domain_adapt_emt.csv")
