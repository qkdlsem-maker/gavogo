#!/usr/bin/env python
"""[5단계-exiD] cross-domain 적응: exiD OOD 개선.
학습: highD+NGSIM+MiTra, 타깃: exiD
실행: python scripts/05_domain_adapt_exid.py"""
import sys
...
def main():
    ...
    for h in config.HORIZONS_SEC:
        ...
        exid = load("exiD", h)
        cols = [c for c in COLS if c in src.columns and c in exid.columns]

        Xs, ys = clean(src, cols), src["label"].astype(int)
        Xe, ye = clean(exid, cols), exid["label"].astype(int)
    ...
    out.to_csv(config.TABLES_DIR / "domain_adapt_exid.csv", index=False)
    print("\n저장: domain_adapt_exid.csv")
