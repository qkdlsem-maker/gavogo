...
from src.models.metrics import calibration_bins as ece
...
def calibration_ood():
    ...
    y = src["label"].astype(int).values
    tr, te = gsplit(keys, config.RANDOM_STATE)
    _, med = clean(src.loc[tr], cols)
    X, _ = clean(src, cols, med=med)
    m = fit_xgb(X[tr], y[tr], seed=config.RANDOM_STATE)
    ...
    ax.set_xlabel("Confidence"); ax.set_ylabel("Observed positive fraction")
