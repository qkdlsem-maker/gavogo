...
from src.models.train import fit_xgb, predict_best
    ...
    for t in TGT:
        ...
        for seed in SEEDS:
            ...
            for fr in FRACS:
                ...
                raw_only = tgt.loc[ft, cols].replace([np.inf, -np.inf], np.nan)
                only_median = raw_only.loc[tr_m].median()
                Xonly = raw_only.fillna(only_median)
                Xonly_test = tgt.loc[test, cols].replace([np.inf, -np.inf], np.nan).fillna(only_median)
                b_only = train_boost(Xonly.loc[tr_m], ytr, Xonly.loc[va_m], yva, seed, base=None)
                a_b = roc(yte, predict_best(b_only, xgb.DMatrix(Xonly_test)))

                # (C) source → target: continued training
                b_ft = train_boost(Xtr, ytr, Xva, yva, seed, base=base.get_booster())
                a_c = roc(yte, predict_best(b_ft, dte))
