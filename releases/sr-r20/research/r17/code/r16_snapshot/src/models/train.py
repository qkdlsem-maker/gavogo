...
def predict_best(booster, matrix):
    """Use the validation-selected tree limit when native XGBoost stopped early."""
    try:
        end = booster.best_iteration + 1
    except AttributeError:
        end = booster.num_boosted_rounds()
    return booster.predict(matrix, iteration_range=(0, end))
